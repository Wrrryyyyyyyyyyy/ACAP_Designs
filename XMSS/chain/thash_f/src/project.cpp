...
int main(int argc, char **argv) 
{
    ...
    uint8_t  *inputbuf_data  = (uint8_t*) GMIO::malloc(2*mlen*sizeof(uint8_t)); //GMIO 只能用这种方式输入.mlen 不一定要4的倍数，但是在这一行，送入的数据必须是32-bit的倍数
    uint8_t *outputbuf_data = (uint8_t*) GMIO::malloc(32*sizeof(uint8_t));

    uint8_t  *inputbuf_addr  = (uint8_t*) GMIO::malloc(mlen*sizeof(uint8_t)); //GMIO 只能用这种方式输入.mlen 不一定要4的倍数，但是在这一行，送入的数据必须是32-bit的倍数
