...
void sha256_mask(input_stream<uint32> * __restrict bufin, /*int len , */ output_stream<uint32>* __restrict bufout)
{
   ...
   //unsigned long long time1=tile.cycles();
    ...
    if (len > 0) {
        memcpy(buf, temp, len); //      readincr 有大小端问题，但是在这里又被纠正了                                                                 
    }
    ...
    //能够得到正确的h和反过来的input，在这里前把h打反，然后再反着发射出去，就正了
