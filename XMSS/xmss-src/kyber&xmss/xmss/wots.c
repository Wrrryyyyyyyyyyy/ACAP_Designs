...
{
#ifdef CHAIN_HARDWARE
//    if ((start == 0) && (steps == params->wots_w - 1))
//    {
        unsigned char addr_bytes[32];  
    
        addr_to_bytes(addr_bytes, addr);
        
        // start step cannot be bigger than end step in gen_chain.v
        if (steps != 0)
          chain_hardware(in, pub_seed, start, steps, addr_bytes, out);
        else
...
//    }
    ...
    for (i = start; i < (start+steps) && i < params->wots_w; i++) {
        ...
        //	printf("chain addr[%d]=",i);
        //for (unsigned int j = 0; j < 8; j++){
 	//	printf("%02x", addr[j]);
 	//} printf("\n\n");
 	
        thash_f(params, out, out, pub_seed, addr);
        //	printf("chain out[%d]=",i);
        //for (unsigned int j = 0; j < params->n; j++){
      	//	printf("%02x", out[j]);     		    		
      	//	}
    	//printf("\n\n");
